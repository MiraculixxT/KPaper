...
import de.miraculixx.mcommons.extensions.toMap
...
class StorageGUI(
    ...
    private val locale = players.first().language()
    private val arrowUpRed = if (scrollable) InventoryUtils.getCustomItem("arrowUpRed", 9000, KHeads.ARROW_UP_RED, locale) else null
    private val arrowDownRed = if (scrollable) InventoryUtils.getCustomItem("arrowDownRed", 9000, KHeads.ARROW_DOWN_RED, locale) else null
    private val arrowUpGreen = if (scrollable) InventoryUtils.getCustomItem("arrowUpGreen", 9001, KHeads.ARROW_UP_GREEN, locale) else null
    private val arrowDownGreen = if (scrollable) InventoryUtils.getCustomItem("arrowDownGreen", 9002, KHeads.ARROW_DOWN_GREEN, locale) else null
